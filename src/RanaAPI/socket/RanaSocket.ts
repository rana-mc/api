...
export default class RanaSocket extends EventEmitter {
  ...
  private init = () => {
    ...
    /** Utility actions. */
    this.on(
      RanaSocketEvents.ClientServerUpdate,
      this.onClientServerUpdate.bind(this)
    );
    this.on(
      RanaSocketEvents.SocketServersFlush,
      this.onSocketServersFlush.bind(this)
    );
  };
  ...
  private onSocketServersFlush(servers: Server[]) {
    this.data = servers;
    this.servers = servers.map((_server: ForgeServerData & FabricServerData) => {
  ...
  private appendListeners(server: RanaServer) {
    ...
    server.on(ServerEvents.EulaChanged, (eula) =>
      this.updateServerEULA(server, eula)
    );
  ...
  private createServer(server: Server) {
    ...
    if (server?.core?.type === ServerCoreType.Forge) {
      _server = new ForgeServer(server as ForgeServerData);
    }

    if (server?.core?.type === ServerCoreType.Fabric) {
      _server = new FabricServer(server as FabricServerData);
    }
